...
- *현재 컴파일 안되는 이슈는 해결되었으니 안심하시고 다운로드 진행하시면 됩니다.*
...
- 주소에 띄어쓰기와 한글을 포함하지 않는 위치에 zip파일을 압축해제한 후 바로 실행 가능합니다.
  - 텍스트 파일을 생성하여 파일 확장자명을 `.stm`으로 설정하고 연결 프로그램으로 STaR_TERM.exe를 선택하세요. 기본 실행 파일로는 STaR_TERM.exe를 사용합니다.
  ...
  - `.stm`파일을 실행하면 매크로 파일이 컴파일된 후 실행됩니다.
    - 컴파일된 파일은 `.stb`로 저장되며, `.stb` 파일도 STaR_TERM.exe를 연결 프로그램으로 설정하면 컴파일 과정 없이 실행 가능합니다.
...
- Stack 사이즈 조정(Win10에서는 잘 돌아가는데, Win11에서는 오작동이 난다는 제보가 있네요...)
  - 스타라잇 컴파일러가 생각보다 요구되는 Stack 사이즈가 많은 편이라서 컴파일러 최적화가 진행될 예정입니다. 그 전까지는 아래 수칙을 준수하시기 바랍니다.
    - 같은 연산자 7개 이상 사용 자제
    - f문자열 사용 시 내부에 3~4개정도만 사용 권장
    - f문자열끼리 덧셈 시 괄호 추가 권장
  - Starlit 컴파일러가 RBR 함수를 여러번 중첩 사용하면 컴파일 속도가 기하급수적으로 느려지는 이슈가 있습니다. 문자 출력 << 연산자가 RBR 함수 기반이니 << 연산자를 자주 사용할 일 있다면 나눠서 쓰시기 바랍니다.
    - 부적절한 예: CONSOLE << "Hello " << "World " << "Nice " << "to " << "Meet " << "You! " << CONSOLE.ln
    - 적절한 예: CONSOLE << "Hello " << "World " \ CONSOLE << << "Nice " << "to " << "Meet " << "You! " \ CONSOLE << CONSOLE.ln
